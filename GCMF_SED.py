...
class GCMF_SED(object):
    ...
    def define_model(self,user_indices,user_cdcf_indices,item_indices,
        ...
        #self.sed_rep      = tf.nn.dropout(self.sed_rep,self.keep_prob_layer)
        self.gcmf_sed_rep = tf.concat([self.gcmf_rep,self.sed_rep], axis=1)
        
        self.W_GAE = tf.concat([(1-self.alpha) * self.gcmf_model.W_G,
                                self.alpha * self.sed_model.W_AE], axis=0) ##0.5
        #self.pred_rating_gcmf = self.gcmf_model.pred_rating
        #self.pred_rating_sed = self.sed_model.pred_rating
        ...
        #self.pred_rating = ((1-self.alpha) * self.pred_rating_gcmf + 
        #                    self.alpha * self.pred_rating_sed)
