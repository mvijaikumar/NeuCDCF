...
class SDAE(object):
    ...
    def define_model(self,user_indices,user_cdcf_indices,item_indices,
        ...
        self.b_u3 = tf.Variable(tf.zeros(shape=[1,1]),dtype=tf.float32,name='b_u3')
        self.b_u4 = tf.Variable(tf.zeros(shape=[1,1]),dtype=tf.float32,name='b_u4')
        self.b_v1 = tf.Variable(tf.zeros(shape=[1,1]),dtype=tf.float32,name='b_v1')
        self.b_v2 = tf.Variable(tf.zeros(shape=[1,1]),dtype=tf.float32,name='b_v2')
        ...
        self.W_AE = tf.Variable(self.initializer(shape=[self.num_factors,1]),
        ...
        self.Weight_v2    = tf.Variable(self.initializer(shape=[2*self.num_factors,
                            self.num_factors]),dtype=tf.float32,name='Weight_v1')        
        ...
        self.Weight_u2    = tf.Variable(self.initializer(shape=[2*self.num_factors,
                           self.num_factors]),dtype=tf.float32,name='Weight_u2')        
        self.Weight_u3    = tf.Variable(self.initializer(shape=[self.num_factors,
                            2*self.num_factors]),dtype=tf.float32,name='Weight_u3')        
        self.Weight_u4    = tf.Variable(self.initializer(shape=[2*self.num_factors,
                            self.num_target_items]),dtype=tf.float32,name='Weight_u4')
        ...
        self.item_rep2    = tf.nn.sigmoid(tf.matmul(self.item_rep1_dp,
                                                    self.Weight_v2) + self.b_v2)
        self.item_rep2_dp = tf.nn.dropout(self.item_rep2, self.keep_prob_layer)        
        ...
        self.user_rep3    = tf.nn.sigmoid(tf.matmul(
                            self.user_rep2,self.Weight_u3) + self.b_u3) ##
        self.user_rep3_dp = tf.nn.dropout(self.user_rep3, self.keep_prob_layer)        
        self.user_rep4    = self.rating_scale * tf.nn.sigmoid(tf.matmul(
                            self.user_rep3,self.Weight_u4) + self.b_u4)
        
        self.user_target_true  = tf.boolean_mask(self.user_target_embeds,self.user_target_mask)
        self.user_target_pred  = tf.boolean_mask(self.user_rep4,self.user_target_mask)

        self.multiplied_output = tf.multiply(self.user_rep2_dp,self.item_rep2_dp) 
    ...
    def define_loss(self,loss_type='all'):
        ...
        self.recon_error         = tf.reduce_mean(tf.pow(tf.subtract(self.user_target_true,
        ...
        self.regularization_loss = (self.reg_lambda * (tf.nn.l2_loss(self.item_embeds))+
                                    self.reg_Wh *  (tf.nn.l2_loss(self.Weight_u1)+
                                                    ...
                                                    tf.nn.l2_loss(self.Weight_u3)+
                                                    tf.nn.l2_loss(self.Weight_u4)+
                                                    tf.nn.l2_loss(self.Weight_v1)+
                                                    tf.nn.l2_loss(self.Weight_v2)+
